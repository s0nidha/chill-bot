...
async function loadFonts() {
  try {
    ...
    console.log('✅ Polices SpaceMono chargées.');
  } catch (e) {
    console.error('⚠️ Impossible de charger les polices SpaceMono, fallback système utilisé.');
  }
}
...
const levelupCooldowns = new Map();
...
function drawBadge(ctx, x, y, l1, l2) {
  ...
  ctx.fillStyle = C.bg; ctx.font = 'bold 11px SpaceMono, sans-serif'; ctx.textAlign = 'center';
  ctx.fillText(l1, x+65, y+16); ctx.fillText(l2, x+65, y+32);
  ctx.restore();
}
function drawStamp(ctx, x, y, text) {
  ctx.save(); ctx.font = 'bold 11px SpaceMono, sans-serif';
...
function drawSectionLbl(ctx, x, y, text) {
  ...
  ctx.fillStyle = C.blue; ctx.globalAlpha=0.8; ctx.font='10px SpaceMono, sans-serif';
...
async function drawProfileCard(member, xp) {
  ...
  ctx.fillStyle = C.bg; ctx.font = 'bold 11px SpaceMono, sans-serif'; ctx.textAlign='left';
  ctx.fillText('>>>  DOSSIER RH OFFICIEL  —  LES IRRECUPERABLES', pad, 30);
  ctx.save(); ctx.globalAlpha=0.6; ctx.font='10px SpaceMono, sans-serif'; ctx.textAlign='right';
  ...
  ctx.fillStyle=C.bg; ctx.font='bold 22px SpaceMono, sans-serif'; ctx.textAlign='center';
  ctx.fillText(member.displayName[0].toUpperCase(), pad+28, 104); ctx.restore();
  ctx.fillStyle=C.cream; ctx.font='bold 20px SpaceMono, sans-serif'; ctx.textAlign='left';
  ctx.fillText(member.displayName, pad+70, 90);
  drawDot(ctx, role, pad+76, 107, 10);
  ctx.fillStyle=C.yellow; ctx.font='bold 12px SpaceMono, sans-serif';
  ...
  ctx.save(); ctx.fillStyle=C.blue; ctx.globalAlpha=0.8; ctx.font='9px SpaceMono, sans-serif'; ctx.textAlign='center';
  ctx.fillText('ECHELON', pad+sbW/2, sbY+18); ctx.globalAlpha=1;
  ctx.fillStyle=C.yellow; ctx.font='bold 26px SpaceMono, sans-serif';
  ctx.fillText(String(role.level), pad+sbW/2, sbY+50);
  ctx.globalAlpha=0.5; ctx.fillStyle=C.cream; ctx.font='10px SpaceMono, sans-serif';
  ...
  const barY=278;
  ctx.save(); ctx.fillStyle=C.cream; ctx.globalAlpha=0.6; ctx.font='11px SpaceMono, sans-serif'; ctx.textAlign='left';
  ...
  ctx.save(); ctx.fillStyle=C.cream; ctx.globalAlpha=0.35; ctx.font='9px SpaceMono, sans-serif'; ctx.textAlign='left';
...
async function drawLevelUpCard(member, totalXp, oldRole, newRole) {
  ...
  ctx.save(); ctx.fillStyle=C.cream; ctx.globalAlpha=0.75; ctx.font='12px SpaceMono, sans-serif'; ctx.textAlign='left';
  ...
  ctx.save(); ctx.fillStyle=C.cream; ctx.globalAlpha=0.75; ctx.font='12px SpaceMono, sans-serif';
  ctx.fillText(' — ', pad+70+p1+p2, 124);
  const p3 = ctx.measureText(' — ').width; ctx.restore();
  drawDot(ctx, newRole, pad+70+p1+p2+p3+6, 120, 9);
  ctx.fillStyle=C.blue; ctx.font='bold 12px SpaceMono, sans-serif';
  ...
  ctx.fillStyle=C.cream; ctx.font='12px SpaceMono, sans-serif'; ctx.textAlign='center';
  ...
  ctx.fillStyle=C.blue; ctx.font='20px SpaceMono, sans-serif'; ctx.textAlign='center';
  ctx.fillText('->', midX-10, 228);
  drawDot(ctx, newRole, midX+80, 208, 22);
  ctx.fillStyle=C.yellow; ctx.font='bold 13px SpaceMono, sans-serif'; ctx.textAlign='center';
...
async function handleLevelUp(member, channel, totalXp, oldRole, newRole) {
  ...
  setTimeout(() => levelupCooldowns.delete(key), 10_000);
...
client.once('ready', async () => {
  ...
  if (CLIENT_ID) {
    ...
    console.log('⚠️ Variable CLIENT_ID non définie, enregistrement slash sauté.');
  }
});
